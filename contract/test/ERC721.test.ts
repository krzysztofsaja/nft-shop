...
describe('ERC721', () => {
    ...
    const vaultAddress = allConstants.vaultAddress;
    ...
    describe('transferring', async () => {
        const tokenId = 1; // this test will use the first token
    ...
    describe('listing tokens', async () => {
        it('should get base token URI', async () => {
            const result = await myERC721['getBaseURI']();
            expect(result).to.eq(constants.baseURI);
        });

        it('should return number of all minted tokens', async () => {
            const result = await myERC721['getTokenCounter']();
            expect(result).to.eq(10);
        });
    });

    describe('transfer tokens to vault address', async () => {
        it('change the owner of the token to vault address', async () => {
            const ids = [1, 3, 2, 4, 6, 7, 8, 9];
            for (let i = 0; i < ids.length; i++) {
                expect(await myERC721.ownerOf(ids[i])).exist;
                expect(await myERC721.ownerOf(ids[i])).not.to.eq(vaultAddress);
            }
            await expect(myERC721['transferToVault'](ids)).to.emit(myERC721, 'Transfer');
            for (let i = 0; i < ids.length; i++) {
                expect(await myERC721.ownerOf(ids[i])).to.eq(vaultAddress);
            }
        });
    });
});
